...
async function falCheck(statusUrl, responseUrl, falKey, log) {
  // Use ?logs=1 so COMPLETED status response includes output directly
  const statusResp = await fetch(statusUrl + '?logs=0', {
    headers: { 'Authorization': `Key ${falKey}` },
  });
  const statusText = await statusResp.text();
  log(`fal status (${statusResp.status}): ${statusText.slice(0, 300)}`);
  ...
  // Try fetching result from response_url (GET, no body)
  try {
    ...
    // Result fetch failed — try the status_url with ?include_output=1 fallback
    log(`Result fetch failed (${resultResp.status}), trying status with output...`);
  } catch(e) {
    log('Result fetch error:', e.message);
  }

  // Last resort: re-fetch status with logs to see if output is embedded
  try {
    const statusResp2 = await fetch(statusUrl + '?logs=1', {
      headers: { 'Authorization': `Key ${falKey}` },
    });
    if (statusResp2.ok) {
      const data = await statusResp2.json();
      log(`Status with logs output keys: ${Object.keys(data).join(',')}`);
      // Some fal models embed output in status response
      if (data.output) return { status: 'COMPLETED', output: data.output };
      if (data.response) return { status: 'COMPLETED', output: data.response };
    }
  } catch(e) {}
...
async function submitMerge(trimmedUrls, falKey, log) {
  ...
  log(`Merge submit full response (${resp.status}): ${text}`);
  if (!resp.ok) throw new Error(`Merge submit failed: ${text}`);
  const data = JSON.parse(text);
  const reqId = data.request_id;
  // fal may or may not include status_url/response_url — construct them if missing
  // Pattern confirmed: fal-ai/ffmpeg-api (no subpath) for status/result
  const BASE = 'https://queue.fal.run/fal-ai/ffmpeg-api';
  return {
    request_id: reqId,
    status_url: data.status_url || `${BASE}/requests/${reqId}/status`,
    response_url: data.response_url || `${BASE}/requests/${reqId}`,
  };
}
...
async function storeAndFinish(supabase, job, videoUrl, clipUrls, log) {
  ...
  try {
    ...
    } catch(insertErr) { log('axel_exports insert failed (non-fatal):', insertErr.message); }
