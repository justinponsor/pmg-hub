// api/axel-status.js — Polls fal.ai queue and drives the pipeline forward
  ...
  try {
    ...
    if (job.status === 'trimming' && predMeta?.type === 'trim_batch') {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        const { status, output } = await falCheckResult(
          'fal-ai/workflow-utilities/trim-video', ids[i], FAL_KEY, log
        );
        log(`Trim ${i + 1}: ${status}`);

        if (status === 'COMPLETED' && output?.video?.url) {
          trimmedUrls[i] = output.video.url;
          log(`Trim ${i + 1} done: ${trimmedUrls[i]}`);
        } else if (status === 'FAILED') {
          log(`Trim ${i + 1} failed, falling back to original`);
      ...
      if (completedCount === clipUrls.length) {
        log('All trims complete, advancing...');
        if (trimmedUrls.length === 1) {
          await startMusicOrFinish(supabase, job, trimmedUrls[0], FAL_KEY, log);
        } else {
          const mergeId = await submitMerge(trimmedUrls, FAL_KEY, log);
          await supabase.from('axel_jobs').update({
            status: 'merging',
            trimmed_urls: JSON.stringify(trimmedUrls),
            replicate_prediction_id: JSON.stringify({ type: 'merge', id: mergeId }),
          }).eq('id', jobId);
        }
      }
    ...
    if (job.status === 'merging' && predMeta?.type === 'merge') {
      const { status, output } = await falCheckResult(
        'fal-ai/ffmpeg-api/merge-videos', predMeta.id, FAL_KEY, log
      );
      log(`Merge: ${status}`);
      ...
      if (status === 'COMPLETED') {
        ...
        if (url) {
          ...
          await supabase.from('axel_jobs').update({
            status: 'failed', error: `Merge produced no URL. Output: ${JSON.stringify(output)}`,
          }).eq('id', jobId);
        }
      } else if (status === 'FAILED') {
        await supabase.from('axel_jobs').update({
...
// ── fal.ai: check status, fetch result if COMPLETED ──
async function falCheckResult(modelId, requestId, falKey, log) {
  // Step 1: check status
  ...
  log(`fal status raw (${statusResp.status}): ${statusText.slice(0, 200)}`);
  ...
  // Step 2: fetch result (status endpoint doesn't include output)
  const resultResp = await fetch(
    `https://queue.fal.run/${modelId}/requests/${requestId}`,
    { headers: { 'Authorization': `Key ${falKey}` } }
  );
  const resultText = await resultResp.text();
  log(`fal result raw (${resultResp.status}): ${resultText.slice(0, 300)}`);
