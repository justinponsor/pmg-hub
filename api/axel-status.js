    ...
    if (job.status === 'merging' && predMeta?.type === 'merge') {
      ...
      if (status === 'COMPLETED') {
        ...
        if (url) {
          ...
          // Output null — fall back to last trimmed clip
          log('Merge output null. Raw output:', JSON.stringify(output));
          const trimmedUrls = safeJson(job.trimmed_urls) || [];
          const fallbackUrl = trimmedUrls[trimmedUrls.length - 1] || trimmedUrls[0];
          log('Using fallback trimmed URL:', fallbackUrl);
          await startMusicOrFinish(supabase, job, fallbackUrl || job.current_video_url, FAL_KEY, log);
        }
      } else if (status === 'FAILED') {
