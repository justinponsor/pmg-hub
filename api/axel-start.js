...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ── STEP 2: Scale all clips to target format (cover-crop, no stretch) ──
    const formatLower = format.toLowerCase();
    let targetW = 1080, targetH = 1920; // default: 9:16 reel
    if (formatLower.includes('16:9') || formatLower.includes('landscape') || formatLower.includes('youtube')) {
      targetW = 1920; targetH = 1080;
    } else if (formatLower.includes('1:1') || formatLower.includes('square')) {
      targetW = 1080; targetH = 1080;
    }
    log(`Scaling clips to ${targetW}x${targetH} (crop mode)...`);
    const scaledResults = await Promise.all(
      trimResults.map(async (url, i) => {
        try {
          const result = await fal.subscribe('fal-ai/workflow-utilities/scale-video', {
            input: { video_url: url, width: targetW, height: targetH, scaling_mode: 'crop' },
          });
          const scaledUrl = result.data?.video?.url;
          if (scaledUrl) { log(`Clip ${i + 1} scaled: ${scaledUrl}`); return scaledUrl; }
          log(`Clip ${i + 1} scale no URL, using trimmed`);
          return url;
        } catch(e) {
          log(`Clip ${i + 1} scale error: ${e.message}, using trimmed`);
          return url;
        }
      })
    );
    log('All clips scaled:', scaledResults);

    // ── STEP 3: Merge all scaled clips ──
    let mergedUrl = scaledResults[0];
    if (trimResults.length > 1) {
      try {
        log('Merging clips...');
        const mergeResult = await fal.subscribe('fal-ai/ffmpeg-api/merge-videos', {
          input: { video_urls: scaledResults },
        });
    ...
    log('Adding auto-captions...');
    try {
      const captionResult = await fal.subscribe('fal-ai/workflow-utilities/auto-subtitle', {
        input: { video_url: mergedUrl },
      });
      const captionedUrl = captionResult.data?.video?.url;
      if (captionedUrl) {
        mergedUrl = captionedUrl;
        log('Captions added:', mergedUrl);
      } else {
        log('Captions no URL, continuing without captions');
      }
    } catch(e) {
      log('Captions error (non-fatal):', e.message);
    }

    // ── STEP 4: Create job + kick off music ──
    const { data: job, error: insertError } = await supabase
      ...
      .insert({
        ...
        trimmed_urls: JSON.stringify(scaledResults),
